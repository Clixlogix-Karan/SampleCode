...
const LoggedIn = ({ value,sortBy, offersLists,handleSortBy, acceptCallback, activitiesList, TabPanel, handleSearch, router, handleLikeUnlike, unFollowHandler, FollowHandler, userDetails, user, searchName, disableMore, label, collectedNfts, handleChange, handleChangeTab, a11yProps, open, anchorEl, handleClose, expanded, handleClick, createdItemList, itemsOnSale, collectedFavNfts, loadMore }) => {
            ...
            <div className="d-flex align-items-center sm-space-between sm-t-20 profile">
              <div className="latestFilter">
                <div className="custom-select">
                  <select  defaultValue={'last'} onChange={(e)=>handleSortBy(e.target.value)}>
                    <option style={{background:'#2d2258',color:'white'}} value="first">Latest First</option>
                    <option style={{background:'#2d2258',color:'white'}} value="last">Latest Last</option>
                  </select>
                </div>
              </div>
                    ...
                    return (
                      <li className="collection-box-container">
                        ...
                        <div className="collection-item-wrp" >
                          <div className="item-img-wrp" onClick={() => router.push(itemsCollections?.creatorData?._id == user?._id ? `/item/${itemsCollections._id}` : `/nft-art/${itemsCollections?._id}`)}>
                            <a>
                              <figure>
                          ...
                          <div className="colllection-caontent">
                            ...
                            <div className="collection-btm">
                              <h3> {shortStr(itemsCollections.title)}</h3>
                              <h4>{(itemsCollections?.description + "")?.length < 10 ? itemsCollections?.description : (itemsCollections?.description + "")?.substring(0, 10) + "..."}</h4>
                              <span style={{ color: '#fff' }}>by <strong><Link href={`/profile/${itemsCollections?.creatorData?._id}`} ><a>{shortStr(itemsCollections?.creatorData?.name || itemsCollections?.creatorData?.walletAddress)}</a></Link></strong></span>
                              <h4>
                                <span>{itemsCollections.saleType !== 'bid' ? 'Fixed Price' : 'Top Bid'} - </span> {objGetFromArray(itemsCollections?.topBidData)?.price ? objGetFromArray(itemsCollections?.topBidData)?.price : itemsCollections?.price ? itemsCollections?.price : 0} {' '}<strong>{itemsCollections?.networkData?.nativeCurrency?.name}</strong>
                              </h4>
                              <h3 style={{color:'white',fontWeight:'bold',fontSize:12,margin:0}}>{objGetFromArray(itemsCollections?.item_auction)?.startDate?<span>Auction starts in: <Countdown date={parseDate(objGetFromArray(itemsCollections?.item_auction)?.startDate)} /></span>:null}</h3>
                            </div>
                            <div className="buy-row d-flex justify-content-between align-items-center">
                              {
                                itemsCollections?.creatorData?._id == user?._id ? (
                                  ...
                                  <Link href={`/nft-art/${itemsCollections?._id}`}>
                                    <a
                                      className="by-now-btn collection"
                                    // href="/nft-art"
                                    >
                              ...
                              <div className="fav-col">
                                ...
                                <span className="fav-count">
                                  {shortNum(itemsCollections.favCout)}
                                </span>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="collection-item-wrp">
                          ...
                          <div className="colllection-caontent">
                            ...
                            <div className="collection-btm">
                              ...
                              <h3 style={{color:'white',fontWeight:'bold',fontSize:12,margin:0}}>{objGetFromArray(itemsCollections?.item_auction)?.endDate?<span>Auction ends in: <Countdown date={parseDate(objGetFromArray(itemsCollections?.item_auction)?.endDate)} /></span>:null}</h3>
                            </div>
                            <div className="buy-row d-flex justify-content-between align-items-center">
                              {
                                itemsCollections?.creatorData?._id == user?._id ? (
                ...
                <ul>
                  {collectedFavNfts?.map((itemsCollections) => {
                    return (
                      <li className="collection-box-container">
                        <div className="collection-item-wrp">
                          <div className="item-img-wrp" onClick={() => router.push(itemsCollections?.creatorData?._id == user?._id ? `/item/${itemsCollections?.itemDetails?._id}` : `/nft-art/${itemsCollections?.itemDetails?._id}`)}>
                            <a>
                              <figure>
                                <img src={itemsCollections.tokenData.imageUrl} style={{ height: 280 }} />
                              </figure>
                          ...
                          <div className="colllection-caontent">
                            ...
                            <div className="collection-btm">
                              <h3>{(itemsCollections.title + "")?.length < 10 ? itemsCollections.title : (itemsCollections.title + "")?.substring(0, 10) + "..."}</h3>
                              <h4>{(itemsCollections?.itemDetails?.description + "")?.length < 10 ? itemsCollections?.itemDetails?.description : (itemsCollections?.itemDetails?.description + "")?.substring(0, 10) + "..."}</h4>
                              <span style={{ color: '#fff' }}>by <strong><Link href={`/profile/${itemsCollections?.creatorData?._id}`} ><a>{shortStr(itemsCollections?.creatorData?.name || itemsCollections?.creatorData?.walletAddress)}</a></Link></strong></span>
                              ...
                              <h3 style={{color:'white',fontWeight:'bold',fontSize:12,margin:0}}>{(objGetFromArray(itemsCollections?.item_auction)?.endDate && objGetFromArray(itemsCollections?.item_auction)?.startDate)?parseDate(objGetFromArray(itemsCollections?.item_auction)?.startDate)>new Date()?<span>Auction starts in: <Countdown date={parseDate(objGetFromArray(itemsCollections?.item_auction)?.startDate)} /></span>:<span>Auction ends in: <Countdown date={parseDate(objGetFromArray(itemsCollections?.item_auction)?.endDate)} /></span>:null}</h3>
                            </div>
                            <div className="buy-row d-flex justify-content-between align-items-center">
                              {
                                itemsCollections?.creatorData?._id == user?._id ? (
                                  <div className="left-col" onClick={() => router.push(`/item/${itemsCollections?.itemDetails?._id}`)}>
                                    <a
                                      className="by-now-btn collection"
                                    // href="/nft-art"
                                    >
                                  ...
                                  <Link href={`/nft-art/${itemsCollections?.itemDetails?._id}`}>
                                    <a
                                      className="by-now-btn collection"
                                    // href="/nft-art"
                                    >
              ...
              <div className={styles.profileTableData}>
                <TableContainer>
                  <Table aria-label="simple table">
                    ...
                    <TableBody>
                      {offersLists?.length > 0 && offersLists?.map((item, key) => <OffersItem acceptCallback={acceptCallback} key={key} item={item} />)}
                    </TableBody>
                  </Table>
                </TableContainer>
              </div>
