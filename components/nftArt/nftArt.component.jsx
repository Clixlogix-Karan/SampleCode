...
import { FETCH_ITEM_DETAIL, BUY_NFT_ITEM, PLACE_BID_NFT_ITEM, FETCH_COLLECTION ,CANCEL_BID_NFT_ITEM,OFFERS_BIDS_LIST,OFFERS_SALES_LIST,VISITER} from "../../apis/variables";
...
export default function NftArtComponent() {
  ...
  React.useEffect(() => {
    if (router.query?.userId) {
      ...
      createPostReq(`${FETCH_ITEM_DETAIL}?id=${router.query.userId}`, { ...payload }).then((res) => {
        if (res?.data) {
              ...
              <div className={styles.nftArtRow}>
                <div className={styles.nftArtLeftSideBar}>
                  <div className={styles.artImageContainer}>
                    <figure>
                      <img src={itemDetails?.tokenId?.imageUrl ? itemDetails?.tokenId?.imageUrl : "/images/placeholder.png"} />
                    </figure>
                  </div>
                  <div className={styles.nftArtRightSideBar}>
                    <div className={styles.nftOrgheading}>
                      ...
                      <h5>
                        <Link href={`/explore/${itemDetails?.collectionId?._id}`}>
                          <a>{itemDetails?.collectionId?.name}</a>
                        </Link>
                        {itemDetails?.userId?.isVerified && <span>
                        ...
                        <li>
                          <Grid spacing={2} container>
                            <Grid item>
                              ...
                              <span><Link href={`/profile/${itemDetails?.userId?._id}`}><a>{itemDetails?.userId?.name}</a></Link></span>
                            </Grid>
                            <Grid item>
                        ...
                        <div className="artTabWrp">
                          <div className="artDis">
                            <Grid container spacing={2}>
                              ...
                              <Grid item lg={4}>
                                <p>Contract Address - <span style={{ color: '#986efa', fontWeight: 'bold', cursor: 'pointer' }} onClick={() => copyToClipBoard(itemDetails?.tokenId?.address)} >{(itemDetails?.tokenId?.address + "")?.length < 18 ? itemDetails?.tokenId?.address : (itemDetails?.tokenId?.address + "")?.substring(0, 18) + "..."}</span></p>
                              </Grid>
                              <Grid item lg={2}>
                                <p>Token - <span style={{ color: '#fff', fontWeight: 'bold' }}>ERC - {itemDetails?.collectionId?.collectionType == 'single' ? '721' : '1155'} </span></p>
                              </Grid>
                              <Grid item lg={2}>
                                <p>Token ID - <span style={{ color: '#fff', fontWeight: 'bold', cursor: 'pointer' }} onClick={() => copyToClipBoard(itemDetails?.tokenId?.tokenNo)}>{itemDetails?.tokenId?.tokenNo ? itemDetails?.tokenId?.tokenNo : '--'} </span></p>
                              </Grid>
                            </Grid>
                          </div>
